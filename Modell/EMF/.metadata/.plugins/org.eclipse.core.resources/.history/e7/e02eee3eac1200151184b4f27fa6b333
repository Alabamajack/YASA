...
public class ShootingmachineemfmodelExample {
	...
	public static void main(String[] args) {
        ...
        try {
        	...
        	//Array fuer Namen der Bricks definieren
            String[] Bricks = new String[mySystem.getHasBrick().size()];

//            Brickcount = 0;
//            for(shootingmachineemfmodel.Brick B : mySystem.getHasBrick())
//            {
//            	Bricks[Brickcount] = B.getName();
//            	Brickcount ++;
//            }
            ...
            for(int i = 0; i<mySystem.getHasBrick().size(); i++)
            {
            	...
            	for(int j = 0; j < mySystem.getHasBrick().get(i).getHasTaskBrick().size(); j++)
            	{
            		...
            		for(int k = 0; k < mySystem.getHasBrick().get(i).getHasAlarmBrick().size(); k++)
                	{
                		String Alarm = "\tALARM " + mySystem.getHasBrick().get(i).getHasAlarmBrick().get(k).getName() + "\n"
                				+ "\t{\n"
                				+ "\t\tCOUNTER = " + mySystem.getHasBrick().get(i).getHasAlarmBrick().get(k).getName() + ";\n"
                				+ "\t\tACTION = ACTIVATETASK\n"
                				+ "\t\t{\n"
                				+ "\t\t\tTASK = " + mySystem.getHasBrick().get(i).getHasTaskBrick().get(j).getName() + ";\n"
                				+ "\t\t};\n";
                		if(mySystem.getHasBrick().get(i).getHasAlarmBrick().get(k).isAUTOSTART() == true)
                		{
                			Alarm = Alarm + "\t\tAUTOSTART = TRUE\n"
                					+ "\t\t{\n"
                					+ "\t\t\tALARMTIME = " + mySystem.getHasBrick().get(i).getHasAlarmBrick().get(k).getALARMTIME() + ";\n"
                					+ "\t\t\tCYCLETIME = " + mySystem.getHasBrick().get(i).getHasAlarmBrick().get(k).getCYCLETIME() + ";\n"
                					+ "\t\t\tAPPMODE = LEGOSAR;\n"
                					+ "\t\t};\n";
                		}
                		else
                		...
                		String Counter = "\tCOUNTER " + mySystem.getHasBrick().get(i).getHasAlarmBrick().get(k).getName() + "\n"
                				+ "\t{\n"
                				+ "\t\tMINCYCLE = " + mySystem.getHasBrick().get(i).getHasAlarmBrick().get(k).getMINCYCLE() + ";\n"
                				+ "\t\tMAXALLOWEDVALUE = " + mySystem.getHasBrick().get(i).getHasAlarmBrick().get(k).getMAXALLOWEDVALUE() + ";\n"
                				+ "\t\tTICKSPERBASE = " + mySystem.getHasBrick().get(i).getHasAlarmBrick().get(k).getTICKSPERBASE() + ";\n"
                				+ "\t};\n\n";
