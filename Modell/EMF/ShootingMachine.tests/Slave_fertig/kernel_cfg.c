/*
 *  kernel_cfg.c
 *  Tue Jul 07 17:35:35 2015
 *  SG Version 2.00
 *  sg.exe Slave_fertig.oil -os=ECC2 -I../../../../../../../../nxtOSEK/ecrobot/../toppers_osek/sg/impl_oil -template=../../../../../../../../nxtOSEK/ecrobot/../toppers_osek/sg/lego_nxt.sgt
 */
...
const TaskType SchussMotor = 1;
const TaskType TASK_BT_INTERFACE_READER = 2;
const TaskType TASK_BT_INTERFACE_WRITER = 3;
const TaskType BT_IMPLIZIT_SLAVE = 4;
const TaskType BT_IMPLIZIT_SLAVE2 = 5;
const TaskType StartTrigger = 6;
const TaskType InitHook = 7;

extern void TASKNAME( Trigger )( void );
extern void TASKNAME( SchussMotor )( void );
...
extern void TASKNAME( StartTrigger )( void );
extern void TASKNAME( InitHook )( void );

static __STK_UNIT _stack_Trigger[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_SchussMotor[__TCOUNT_STK_UNIT(512)];
...
static __STK_UNIT _stack_StartTrigger[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_InitHook[__TCOUNT_STK_UNIT(512)];

const Priority tinib_inipri[TNUM_TASK] = { TPRI_MINTASK + 2, TPRI_MINTASK + 3, TPRI_MINTASK + 7, TPRI_MINTASK + 7, TPRI_MINTASK + 9, TPRI_MINTASK + 7, TPRI_MINTASK + 4, TPRI_MINTASK + 10, };
const Priority tinib_exepri[TNUM_TASK] = { TPRI_MINTASK + 2, TPRI_MINTASK + 3, TPRI_MINTASK + 7, TPRI_MINTASK + 7, TPRI_MINTASK + 9, TPRI_MINTASK + 7, TPRI_MINTASK + 4, TPRI_MINTASK + 10, };
...
const FP tinib_task[TNUM_TASK] = { TASKNAME( Trigger ), TASKNAME( SchussMotor ), TASKNAME( TASK_BT_INTERFACE_READER ), TASKNAME( TASK_BT_INTERFACE_WRITER ), TASKNAME( BT_IMPLIZIT_SLAVE ), TASKNAME( BT_IMPLIZIT_SLAVE2 ), TASKNAME( StartTrigger ), TASKNAME( InitHook ), };
const __STK_UNIT tinib_stk[TNUM_TASK] = { (__STK_UNIT)_stack_Trigger, (__STK_UNIT)_stack_SchussMotor, (__STK_UNIT)_stack_TASK_BT_INTERFACE_READER, (__STK_UNIT)_stack_TASK_BT_INTERFACE_WRITER, (__STK_UNIT)_stack_BT_IMPLIZIT_SLAVE, (__STK_UNIT)_stack_BT_IMPLIZIT_SLAVE2, (__STK_UNIT)_stack_StartTrigger, (__STK_UNIT)_stack_InitHook, };
...
const CounterType C_StartTriggerAlarm = 0;
...
const AlarmType StartTriggerAlarm = 0;

DeclareTask(StartTrigger);
static void _activate_alarm_StartTriggerAlarm( void );
static void _activate_alarm_StartTriggerAlarm( void )
{ (void)ActivateTask( StartTrigger ); }

const CounterType alminib_cntid[TNUM_ALARM] = { 0, };
const FP alminib_cback[TNUM_ALARM] = { _activate_alarm_StartTriggerAlarm, };
...
const EventMaskType RTE_Trigger_StartTrigger_GetValue_Event_In_EVENT = (1UL << 2);
const EventMaskType RTE_Trigger_GetValue_Receiver_In_EVENT = (1UL << 3);
const EventMaskType RTE_SchussMotor_GetValue_Event_In_EVENT = (1UL << 4);
const EventMaskType RTE_Schussanlage_Trigger_GetValue_Event_In_EVENT = (1UL << 5);
const EventMaskType BT_IMPLIZIT_SLAVE2_EVENT = (1UL << 6);
